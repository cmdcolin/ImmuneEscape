...
gameplay_font = pygame.font.SysFont(None, 55)
button_font = pygame.font.SysFont(None, 50)
...
text_char = font.render("Player One: Choose Your Character", True, (255,255,255))
textRect_char = text.get_rect()
textRect_char.center = (350, height // 10)

text_path_choice = font.render('You are the PATHOGEN!', True, (255,255,255))
textRect_path = text.get_rect()
textRect_path.center = (500, height // 1.2)

text_imm_choice = font.render('You are the IMMUNE SYSTEM!', True, (255,255,255))
textRect_imm = text.get_rect()
textRect_imm.center = (450, height // 1.2)
...
text_choosepath = font.render("PATHOGEN", True, (255,255,255))
textRect_choosepath = text.get_rect()
textRect_choosepath.center = (400, height // 1.4)

text_chooseimm = font.render("IMMUNE SYSTEM", True, (255,255,255))
textRect_chooseimm = text.get_rect()
textRect_chooseimm.center = (870, height // 1.4)
...
character_sound = pygame.mixer.Sound('data/gameplaysong.wav')
#end screen sounds
...
def draw_character_screen():
    ...
    screen.blit(text_chooseimm, textRect_chooseimm)
    screen.blit(text_choosepath, textRect_choosepath)
...
def draw_gameplay_screen():
    screen.blit(pathogen_background, (0, 0))
    draw_multiline_text(screen, gameplay_text, gameplay_font, (250, 250, 250), (0, 0))
...
continue_button = Button("Continue", width // 2 - 100, height - 100, 200, 50, (255, 255, 255), (255, 0, 0), button_font, (0, 0, 0))
    ...
    if current_state == start_screen:
        ...
        for event in events:
            ...
            if enter_button.mouse(event):
                current_state = gameplay_screen
    if current_state == gameplay_screen:
        sound.set_volume(0.0)
        if not gameplay_music_playing:
            character_sound.play(-1,0)
            gameplay_music_playing = True
        draw_gameplay_screen()
        for event in events:
            if continue_button.mouse(event):
